...
modified:
  - date: 2026-09-24
    actor: architect
    reason: "STORY-187 per-story adversarial pass 7 (F-49, orchestrator decision, following the F-43 precedent) — VP-051's Kani harness (verified SUCCESSFUL) also asserts BC-2.21.006 Postcondition 1 (header_len==10 for rosctr in {Job, Userdata}) and Postconditions 2-4 (big-endian pdu_reference/param_length/data_length reads) and BC-2.21.007 Postcondition 1 (only rosctr in {0x01, 0x02, 0x03, 0x07} reach Some). VP-051 row's Traces clause updated to include BC-2.21.006 and BC-2.21.007 alongside BC-2.21.004/BC-2.21.008/BC-2.21.009 (now BC-2.21.004, BC-2.21.006, BC-2.21.007, BC-2.21.008, BC-2.21.009). Mirrored in VP-INDEX.md (v2.53->2.54) and verification-architecture.md (v2.39->2.40); ARCH-INDEX.md (v2.29->2.30) modified-entry updated. Per-Module Coverage Totals and Overall Totals UNCHANGED (analyzer/s7comm.rs still 5 total: 1 Kani + 3 proptest + 1 fuzz; Grand Totals Kani(19)+proptest(26)+fuzz(4)+integration_unit(6)=55) — source-BC trace addition only, no VP added or retired. Uncommitted; state-manager to commit. Version bump 1.54->1.55."
  - date: 2026-09-24
    actor: architect
    reason: "STORY-187 per-story adversarial pass 5 (F-43, orchestrator decision) — VP-051's Kani harness non-vacuity deliberate-flip check (asserting error_class.is_some() == (rosctr == Ack OR rosctr == AckData) and header_len==12 for Ack/AckData) directly asserts BC-2.21.008 Postcondition 2 (Some result, header_len 12, error_class/error_code populated) and Postcondition 3 (error_class/error_code Some only when rosctr in {Ack, AckData}). VP-051 row's Traces clause updated to include BC-2.21.008 alongside BC-2.21.004/BC-2.21.009. Mirrored in VP-INDEX.md (v2.52->2.53) and verification-architecture.md (v2.38->2.39); ARCH-INDEX.md (v2.28->2.29) modified-entry updated. Per-Module Coverage Totals and Overall Totals UNCHANGED (analyzer/s7comm.rs still 5 total: 1 Kani + 3 proptest + 1 fuzz; Grand Totals Kani(19)+proptest(26)+fuzz(4)+integration_unit(6)=55) — source-BC trace addition only, no VP added or retired. Uncommitted; state-manager to commit. Version bump 1.53->1.54."
  - date: 2026-09-24
    actor: architect
    reason: "STORY-187 canonical-frame holdout human ruling (2026-09-24) — ADR-0014 gains a new dated reconciliation note under Decision 9 item 3 (docs/adr/0014-...md, STORY-187 worktree, commit 025bd47a): real captured Setup Communication Ack_Data evidence (cnblogs; corroborated by Yiqisoft and Inductive Automation KB) shows the parameter block starts at byte 12, not byte 10 — ROSCTR 0x02 (Ack) AND 0x03 (Ack_Data) both carry the 12-byte header with error_class=data[10]/error_code=data[11]; Job (0x01)/Userdata (0x07) remain 10-byte, no error fields. VP-051 row updated: header_len selection is 12 when rosctr in {Ack, AckData}, 10 otherwise; non-vacuity harness requirement corrected to assert error_class.is_some() == (rosctr == Ack OR rosctr == AckData), not the narrower Ack-only formula. Mirrored in VP-INDEX.md (v2.51->2.52) and verification-architecture.md (v2.37->2.38); ARCH-INDEX.md (v2.27->2.28) modified-entry updated. VP counts UNCHANGED (total 55). NOTE: this burst tracks the ARCHITECTURAL ruling only; product-owner is amending BC-2.21.006/007/008/009 in parallel to match — BC text is out of scope for this pass. Version bump 1.52->1.53."
  - date: 2026-09-24
    actor: architect
    reason: "STORY-187 architecture sync for human-ratified rulings (per-story adversarial pass 1) — F-01/F-02/F-12/F-14 reconciled into ADR-0014 (dated reconciliation notes under Decision 2 and Decision 9 item 3, commit f5f10ca1 in the STORY-187 worktree) and mirrored here: VP-053 VP-to-Module row REWORDED (title extended to 'sticky-classification gating'; text now states a protocol_id:None DT frame does NOT classify — F-02 — and classic dissection is gated on the flow's STICKY classified_protocol==Classic, not the current frame's raw protocol_id byte — F-12); VP-051 row CONFIRMED Kani P0 (F-14; supersedes ADR-014 Decision 9 item 3's prior cargo-fuzz-only text) with non-vacuity (DF-KANI-NONVACUITY-001) + bounded-input/unwind harness requirement noted. Prose block (VP-048..VP-055 section) updated to match. Mirrors VP-INDEX v2.51 and verification-architecture.md v2.37. NOTE: tracks the ARCHITECTURAL ruling only; product-owner is amending BC-2.21.001/002/004/005/008/009 in parallel — BC text out of scope for this pass. Per-Module Coverage Totals and Overall Totals UNCHANGED (analyzer/s7comm.rs still 5 total: 1 Kani + 3 proptest + 1 fuzz; Grand Totals Kani(19)+proptest(26)+fuzz(4)+integration_unit(6)=55). Deferred architectural note (assessment only, not fixed): adversary D-01 — SS-20/SS-21 do not model COTP DT segmentation (EOT bit ignored); a multi-DT-TPDU-split S7 PDU would trip BC-2.21.009 and emit a false T0814; recommend a future ADR-014 amendment or dedicated story own this. Version bump 1.51→1.52."
  - date: 2026-09-24
    actor: architect
  ...
  - date: 2026-09-24
    actor: architect
    reason: "STORY-187 spec pass; consistency audit re-check (E-23, feature-s7comm; verified against develop @294174f5 `src/analyzer/s7comm.rs::on_data`; POLICY 9 sync with VP-INDEX v2.49 / verification-architecture.md v2.35). VP-050 row RESCOPED: 'residual bounded at MAX_S7_ISO_ON_TCP_CARRY_BYTES=65,535 with clear-and-1-per-direction-dedup overflow handling' corrected to a REACHABLE-BOUND INVARIANT — residual.len() provably `<=65,534` bytes for every finite on_data call sequence (the literal 65,535 boundary is itself unrealizable via on_data) with NO T0814 finding ever emitted on real on_data traffic; folds in BC-2.20.014's previously-deferred reachability-property VP allocation (formal counterpart of BC-2.20.014 v1.2 Invariant 1; no new VP allocated). The overflow guard's clear/resync/one-T0814-per-direction/dedup REACTION MECHANICS are named OUT OF SCOPE for this on_data-driven property — defense-in-depth, covered instead by synthetic direct-field-injection unit tests (test_BC_2_20_014_at_bound_residual_no_overflow, test_BC_2_20_014_overflow_clear_resync_one_t0814_per_direction, test_BC_2_20_014_repeated_overflow_dedup_same_direction, test_BC_2_20_014_overflow_dedup_independent_per_direction); VP-050's own on_data-reachable claim is named as exercised live by test_BC_2_20_014_live_near_bound_residual_reachable (NOT YET WRITTEN — to be added by the STORY-186 follow-up fix PR; see STORY-186 v1.2 AC-186-004(b)) and test_BC_2_20_014_overflow_unreachable_via_on_data. VP-055 row RESCOPED for the same defect: 'carry buffers remain bounded at 65,535 bytes' corrected to '<=65,534 bytes (on_data-reachable maximum, not the literal guard-constant boundary)', with a clause added that the fuzz harness never emits a T0814 finding. Module/tool/phase/status assignments and Per-Module Coverage Totals UNCHANGED (analyzer/iso_on_tcp.rs 3 total incl. VP-050 proptest; analyzer/s7comm.rs 5 total incl. VP-055 cargo-fuzz); Overall Totals UNCHANGED at 55. Same-day follow-up (coordinator-requested final-audit finding, no version bump): both occurrences of `test_BC_2_20_014_live_near_bound_residual_reachable` in this file (this modified-log entry + VP-050 VP-to-Module table row) tagged with '(NOT YET WRITTEN — to be added by the STORY-186 follow-up fix PR; see STORY-186 v1.2 AC-186-004(b))' — the test does not yet exist on develop (only `test_BC_2_20_014_overflow_unreachable_via_on_data` exists), matching STORY-186's own framing. Version bump 1.49→1.50."
  - date: 2026-09-06
    actor: spec-steward
...
| VP-050 | TPKT/COTP carry-buffer walk-first-residual-bound reassembly, direction isolation, and on_data-reachable-bound invariant: walk-first anti-evasion (frame-walk extracts every complete frame before any bound is applied to the residual); carry_c2s/carry_s2c never mixed; REACHABLE-BOUND INVARIANT (folds in BC-2.20.014's reachability-property VP allocation — formal counterpart of BC-2.20.014 v1.2 Invariant 1; no new VP allocated) — for every finite sequence of on_data calls, residual.len() is provably <=65,534 bytes (strictly below the MAX_S7_ISO_ON_TCP_CARRY_BYTES=65,535 guard constant, which is itself unrealizable via on_data), and consequently NO T0814 carry-overflow finding is ever emitted on the real on_data path; resync advances exactly 1 byte/iteration (termination guarantee). OUT OF SCOPE for this on_data-driven property: the overflow guard's clear/resync/one-T0814-per-direction/dedup REACTION MECHANICS — defense-in-depth, unreachable via on_data, exercised only by synthetic direct-field-injection unit tests (test_BC_2_20_014_at_bound_residual_no_overflow, test_BC_2_20_014_overflow_clear_resync_one_t0814_per_direction, test_BC_2_20_014_repeated_overflow_dedup_same_direction, test_BC_2_20_014_overflow_dedup_independent_per_direction); VP-050's own on_data-driven claim is exercised live by test_BC_2_20_014_live_near_bound_residual_reachable, test_BC_2_20_014_live_near_bound_residual_single_call, and test_BC_2_20_014_overflow_unreachable_via_on_data. ADR-014 Decision 8 (WALK-FIRST-RESIDUAL-BOUND). Mirrors VP-045/VP-035/VP-033. RESCOPED STORY-187 spec pass, consistency audit re-check (2026-09-24; verified against develop @294174f5). | analyzer/s7comm.rs | proptest | P1 | draft |
| VP-051 | S7comm header bounds-before-slice safety: parse_s7comm_header never panics for data.len()<10; caller-side param_length/data_length bounds-check-before-slice enforced (no OOB slice for any adversarial values); header_len+param_length+data_length cannot overflow usize. header_len is 12 when rosctr in {Ack, AckData} (error_class/error_code Some), 10 otherwise (Job, Userdata; error_class/error_code None) — corrected per canonical-frame holdout ruling, 2026-09-24 (both Ack AND Ack_Data carry the 12-byte error-field header, not Ack alone). Pure-core free fn. ADR-014 Decision 9 (reconciliation note, F-14, STORY-187 adversarial pass 1, 2026-09-24 — CONFIRMS Kani P0, non-vacuous per DF-KANI-NONVACUITY-001 (asserting error_class.is_some() == (rosctr == Ack OR rosctr == AckData)), bounded input/unwind). Mirrors VP-044 (ASDU minimum-length guard). Traces BC-2.21.004, BC-2.21.006, BC-2.21.007, BC-2.21.008, BC-2.21.009 (BC-2.21.008 added — F-43, orchestrator decision, STORY-187 adversarial pass 5, 2026-09-24: the non-vacuity assertion above directly asserts BC-2.21.008 PC2/PC3, error_class.is_some() == rosctr in {Ack, AckData}, header_len 12 for Ack/AckData; BC-2.21.006 and BC-2.21.007 added — F-49, orchestrator decision, STORY-187 adversarial pass 7, 2026-09-24, following the F-43 precedent: the same verified-SUCCESSFUL harness also asserts BC-2.21.006 PC1 (header_len 10 for Job/Userdata) and PC2-4 (big-endian pdu_reference/param_length/data_length reads) and BC-2.21.007 PC1 (only rosctr in {0x01, 0x02, 0x03, 0x07} reach Some)). | analyzer/s7comm.rs | Kani | P0 | draft |
...
| VP-053 | protocol_id four-way dispatch totality, sticky-classification gating, and unclassified never-force-fit: on_data's branch on CotpHeader::protocol_id is exhaustive/mutually-exclusive over None (parse_cotp_header) / ConnectRequest+ConnectConfirm (session-tracking only) / DT+protocol_id:None (does NOT classify — F-02) / Some(0x32) classic / Some(0x72) plus / Some(other) unclassified-gap. LOAD-BEARING: all 254 non-{0x32,0x72} values NEVER attributed to S7comm; no proximity-based fallback; sticky first-classification-wins; classic dissection gated on the flow's STICKY classification, not the current frame's raw protocol_id byte (F-12; ADR-014 Decision 2 reconciliation, STORY-187 adversarial pass 1, 2026-09-24). | analyzer/s7comm.rs | proptest | P0 | draft |
...
| VP-055 | S7comm/ISO-on-TCP combined parse-chain no-panic fuzz (fuzz_s7comm_parser): full TPKT→COTP→S7comm chain never panics/OOBs on arbitrary byte sequences across arbitrarily many on_data calls; carry buffers remain bounded at <=65,534 bytes (the on_data-reachable maximum — NOT the literal 65,535 guard-constant boundary, which is unrealizable via on_data); the fuzz harness never emits a T0814 carry-overflow finding (overflow-reaction mechanics are synthetic-unit-test-only, see VP-050); frame-walk loop terminates (each iteration extracts a complete frame advancing >=4 bytes, or stashes a residual and returns). Mirrors VP-028/VP-047. RESCOPED STORY-187 spec pass, consistency audit re-check (2026-09-24). | analyzer/iso_on_tcp.rs + analyzer/s7comm.rs | cargo-fuzz | P1 | draft |
...
- VP-048..VP-055 (analyzer/iso_on_tcp.rs, analyzer/s7comm.rs / Kani + proptest + cargo-fuzz):
  ...
  guard; CONFIRMED Kani P0 by the STORY-187 adversarial pass 1 reconciliation (F-14,
  2026-09-24) — the ADR-014 Decision 9 item 3 text that previously read "cargo-fuzz
  candidate ... rather than Kani" is superseded; the harness MUST be non-vacuous per
  DF-KANI-NONVACUITY-001 and use bounded input/unwind. VP-052 (proptest P1) and VP-053
  (proptest P0) cover SS-21's classification totality properties — VP-053 is explicitly
  load-bearing (never-force-fit unclassified-gap handling for the port-102 four-way
  protocol collision) and is P0 despite being a proptest (not Kani) property, since the
  classification state depends on stateful S7commFlowState rather than a pure free fn.
  STORY-187 reconciliation (F-02/F-12, 2026-09-24) refines VP-053's scope: a DT frame with
  protocol_id:None does NOT participate in sticky classification (only a Some(byte) DT
  frame classifies), and classic dissection is gated on the flow's STICKY
  classified_protocol==Classic, not the current frame's raw protocol_id byte — both are
  now explicit clauses of VP-053's property statement (see VP-to-Module table row above).
  VP-054 (proptest P1) covers the download/upload structural
