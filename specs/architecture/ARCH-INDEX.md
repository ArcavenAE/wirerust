...
modified:
  - date: 2026-09-24
    actor: architect
    reason: "STORY-187 per-story adversarial pass 7 (F-49, orchestrator decision, following the F-43 precedent): VP-051's Kani harness (verified SUCCESSFUL) also asserts BC-2.21.006 Postcondition 1 (header_len 10 for rosctr in {Job, Userdata}) and Postconditions 2-4 (big-endian pdu_reference/param_length/data_length reads) and BC-2.21.007 Postcondition 1 (only rosctr in {0x01, 0x02, 0x03, 0x07} reach Some) — BC-2.21.006 and BC-2.21.007 added to VP-051's traced/source BCs in VP-INDEX.md (v2.53->v2.54), verification-architecture.md (v2.39->v2.40), and verification-coverage-matrix.md (v1.54->v1.55), alongside VP-051's existing BC-2.21.004/BC-2.21.008/BC-2.21.009 traces (now BC-2.21.004, BC-2.21.006, BC-2.21.007, BC-2.21.008, BC-2.21.009). VP counts UNCHANGED (no VP added or retired; total 55). NOTE: this is a source-BC trace addition only — no VP-051 property text, tool, phase, or module assignment changed beyond the added BC-2.21.006/BC-2.21.007 cross-references. Uncommitted; state-manager to commit. Version bump 2.29->2.30."
  - date: 2026-09-24
    actor: architect
    reason: "STORY-187 per-story adversarial pass 5 (F-43, orchestrator decision): VP-051's harness asserts BC-2.21.008 Postconditions 2/3 (error_class.is_some() == rosctr in {Ack, AckData}; header_len 12 for Ack/AckData) — BC-2.21.008 added to VP-051's traced/source BCs in VP-INDEX.md (v2.52->v2.53), verification-architecture.md (v2.38->v2.39), and verification-coverage-matrix.md (v1.53->v1.54), alongside VP-051's existing BC-2.21.004/BC-2.21.009 traces. VP counts UNCHANGED (no VP added or retired; total 55). NOTE: this is a source-BC trace addition only — no VP-051 property text, tool, phase, or module assignment changed beyond the added BC-2.21.008 cross-reference. Uncommitted; state-manager to commit. Version bump 2.28->2.29."
  - date: 2026-09-24
    actor: architect
    reason: "STORY-187 canonical-frame holdout human ruling (2026-09-24) — ADR-0014 (docs/adr/0014-...md, STORY-187 worktree, commit 025bd47a) gains a new dated reconciliation note under Decision 9 item 3: real captured Setup Communication Ack_Data evidence (`32 03 00 00 FF FF 00 08 00 00 00 00 F0 00 00 01 00 01 00 F0`, cnblogs; corroborated by Yiqisoft and Inductive Automation KB) shows the S7comm parameter block starts at byte 12, not byte 10 — ROSCTR 0x02 (Ack) AND 0x03 (Ack_Data) both carry the 12-byte header with error_class=data[10]/error_code=data[11]; Job (0x01)/Userdata (0x07) remain 10-byte, no error fields. This corrects the ADR's prior F-14 note (previous entry below), which left the 10-or-12-byte split's rosctr mapping implicit and, read narrowly, could be taken as Ack-only. VP-INDEX.md (v2.51->v2.52), verification-architecture.md (v2.37->v2.38), and verification-coverage-matrix.md (v1.52->v1.53) updated in lockstep: VP-051's Must Prove row and P0-list bullet now state header_len=12 when rosctr in {Ack, AckData}, 10 otherwise, and its non-vacuity harness requirement is corrected to assert error_class.is_some() == (rosctr == Ack OR rosctr == AckData) rather than the narrower Ack-only formula. VP counts UNCHANGED (no VP added or retired; total 55). NOTE: this burst tracks the ARCHITECTURAL ruling only — product-owner is amending BC-2.21.006/007/008/009 in parallel; BC text is out of scope for this pass. Version bump 2.27->2.28."
  - date: 2026-09-24
    actor: architect
    reason: "STORY-187 architecture sync for human-ratified rulings (per-story adversarial pass 1, 2026-09-24) — ADR-0014 (docs/adr/0014-...md, STORY-187 worktree, commit f5f10ca1) gains two dated reconciliation notes: under Decision 2 (F-01 session_established is directional CR/CC-paired, not any-CC; F-02 a DT frame with protocol_id:None does NOT classify — classification occurs only on the first DT carrying Some(byte); F-12 classic S7comm header dissection is gated on the flow's STICKY classified_protocol==Classic, not the current frame's raw protocol_id byte — the ADR's no-misattribution guarantee applied at the flow level) and under Decision 9 item 3 (F-14: VP-051 CONFIRMED as a Kani P0 target for parse_s7comm_header, superseding the item's prior 'cargo-fuzz ... not a Kani P0 target' text; harness MUST be non-vacuous per DF-KANI-NONVACUITY-001 and use bounded input/unwind). VP-INDEX.md (v2.50→v2.51), verification-architecture.md (v2.36→v2.37), and verification-coverage-matrix.md (v1.51→v1.52) updated in lockstep: VP-053 row reworded for the F-02/F-12 dispatch-gating refinement, VP-051 row's Kani-P0 status confirmed with the non-vacuity/bounded-unwind harness note. VP counts UNCHANGED (no VP added or retired; total 55). NOTE: this burst tracks the ARCHITECTURAL ruling only — product-owner is amending BC-2.21.001/002/004/005/008/009 in parallel; BC text is out of scope for this pass. Deferred architectural note (assessment only, not fixed): adversary D-01 — SS-20/SS-21 do not model COTP DT segmentation (the COTP EOT bit is ignored), so an S7 PDU split across multiple DT TPDUs would trip BC-2.21.009's bounds check and emit a false T0814; recommend a future ADR-014 amendment (or a dedicated follow-on story) own DT-segmentation reassembly — MEDIUM severity (false-positive finding on legitimate large-PDU traffic, not a misattribution or safety defect). Version bump 2.26→2.27."
  - date: 2026-09-24
    actor: architect
